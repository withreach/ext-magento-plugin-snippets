...
namespace Reach\Payment\Gateway\Http\Client;
...
class ClientMock implements ClientInterface
{
    ...
    public function placeRequest(TransferInterface $transferObject)
    {
        ...
        $response['session_id'] = $request['REACH_SESSION_ID'];
        $response['increment_id'] = $request['INVOICE'];

        $config = Configuration::getDefaultConfiguration()
            ->setUsername('sdk-team')
            ->setPassword('password');

        $reachSessionApi = new SessionApi(
            new ReachGuzzleClient(),
            $config
        );

        $reachSession = $reachSessionApi->updateSession($request['REACH_SESSION_ID'], $request['REACH_SESSION_MODEL']);

        $this->logger->debug(
            '**************** MAG-85-POC - CLIENT MOCK',
            [
                'request' => $request['REACH_SESSION_MODEL'],
                'response' => $reachSession
            ]
        );
